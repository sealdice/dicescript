...
基本格式为: XaYmZkNqM，X骰数，Y加骰线，Z面数，N阈值(>=)，M阈值(<=)

此规则记录的值是超过面数的骰子个数，默认使用十面骰，例如说 5a9k2，意思是初始骰5个，当有骰的点数大于2时，结果加1，当大于等于9时，再骰一个。如果新骰的又大于等于9，那么继续骰下一轮，以此类推。
...
此外，DiceScript没有布尔类型，true的值为整数1，false的值为整数2
